...
export const getStaticPaths = async () => {
  ...
  const paths = range(1, Math.ceil(repos.totalCount / devideNumber)).map((repo) => `/page/health/${repo}`);
...
export const getStaticProps = async (ctx) => {
  ...
  const data = await client.get({
    endpoint: "blog-data",
    queries: { offset: (id - 1) * devideNumber, limit: devideNumber, filters: "category[equals]health" },
  });
